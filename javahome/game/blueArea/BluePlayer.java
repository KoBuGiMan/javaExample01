...
public class BluePlayer extends DefaultAction {
	...
	public double attackSumBlue() {
		double sum = (bm.minePlusAttack() + bn.plusNomalAttack() + ba.attackerPlusAttack());
		return sum;
	}

	public double costSumBlue() {
		double sum = (bm.minePlusCost() + bn.plusNomalCost() + ba.plusAttackerCost());
		return sum;
	}
	...
	public void buyMenu() {
		...
		System.out.println("///////////////////////////////////////////");
		System.out
				.println("총 획득 코스트: " + "0~" + (bm.getMineCost() * bm.getMineNum() + bn.getNomalCost() * bn.getNomalNum()
						+ ba.getAttackerCost() * ba.getAttackerNum()));
		System.out.println("///////////////////////////////////////////");
		System.out.println();
		System.out.println("현재 턴당 공격력");
		System.out.println("광부:" + 0 + "~" + (bm.getMineAttack() * bm.getMineNum()));
		System.out.println("시민:" + 0 + "~" + (bn.getNomalAttack() * bn.getNomalNum()));
		System.out.println("군인:" + 0 + "~" + (ba.getAttackerAttack() * ba.getAttackerNum()));
		System.out.println("///////////////////////////////////////////");
		System.out.println("총 공격력:" + 0 + "~" + (bm.getMineAttack() * bm.getMineNum()
				+ bn.getNomalNum() * bn.getNomalAttack() + ba.getAttackerAttack() * ba.getAttackerNum()));
		System.out.println("///////////////////////////////////////////");
		System.out.println();
		System.out.println("현재보유금액:" + getblueMoney() + "원");
		...
		if (ans == 1) {
			do {
				...
				if (buyUnit == 1) {
					...
					System.out.println("구매를 종료하면 전투를 시작합니다. 계속하시겠습니까?(1.유닛구매/2.전투시작)");
					ans = sc.nextInt();
				}
	...
	public void blueFirstAttack() {
		...
		System.out.println("블루팀 데미지:" + rp.attackSumRed(rn, rm, ra));
		setBlueLife(getBlueLife() - rp.attackSumRed(rn, rm, ra));
	...
	public void battleMenu() {
		...
		if (ba.getAttackerNum() > ra.getAttackerNum()) {
			blueFirstAttack();
			System.out.println("다음턴으로 넘어가시겠습니까? (1.yes/2.no)");
			int ans = sc.nextInt();
			if(ans == 1) {
				return;
			}
			...
			redFirstAttack();
			System.out.println("코스트 획득으로 넘어가시겠습니까? (1.yes/2.no)");
			int ans = sc.nextInt();
			if(ans == 1) {
				return;
			}
	...
	public void addPlayerList() {
		...
		for(int i=0;i<getPlayerList().length;i++) {			
			System.out.println("이름: " + getPlayerList()[i][0] + " / (" + getPlayerList()[i][1] + ")");
		}

	}

	public void addCost(RedPlayer rp) {
		System.out.println();
		System.out.println("<<<<<< 코스트 획득 >>>>>>");
		System.out.println();
		System.out.println("블루팀 획득 코스트:" + costSumBlue());
		setblueMoney(getblueMoney() + costSumBlue());
		System.out.println("현재 블루팀 코스트:" + getblueMoney());

		rp.setredMoney(rp.costSumRed(rn, rm, ra) + rp.getredMoney());
		System.out.println("레드팀 획득 코스트:" + rp.costSumRed(rn, rm, ra));
		System.out.println("---------------------------------------");
		System.out.println("다음턴으로 넘어가시겠습니까? (1.yes/2.no)");
	}
